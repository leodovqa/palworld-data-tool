...
function parsePals() {
  ...
  lines.forEach((line) => {
    ...
    if (parts.length > 3) {
      ...
      const simpleMatches = workText.match(
        /([A-Za-z]+(?:\s+[A-Za-z]+)*?)\s+Lv\.\s+(\d+)/g,
      );
      if (simpleMatches) {
    ...
    const passiveInfo = passiveLogic.getPassiveSets(
      palNum,
      elements.length > 0 ? elements[0] : "Neutral",
      palType,
    );
    ...
    const damageSet =
      passiveInfo && passiveInfo.damage
        ? passiveInfo.damage
        : { best4: [], alternative: "" };
    const mountSet =
      passiveInfo && passiveInfo.mount ? passiveInfo.mount : null;
    ...
    const flat = {
      ...
      moveAlt:
        primaryUse === "mount" && mountSet ? mountSet.alternative || "" : "",
